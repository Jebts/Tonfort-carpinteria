#!/usr/bin/env bash
# vps-reset.sh — Reinicia los contenedores en el VPS (down + up).
set -euo pipefail
...
echo "=== Reiniciando contenedores en $REMOTE:$VPS_REMOTE_DIR ==="
ssh -p "$VPS_SSH_PORT" $SSH_KEY_OPT "$REMOTE" "cd $VPS_REMOTE_DIR && docker compose --env-file .env down"
ssh -p "$VPS_SSH_PORT" $SSH_KEY_OPT "$REMOTE" "cd $VPS_REMOTE_DIR && docker compose --env-file .env up -d"
